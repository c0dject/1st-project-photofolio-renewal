...
const createUser = async (req: Request, res: Response) => {
  ...
  // FIXME 또 오류
  const profile_image = req.file: { location: any }.location;

  const REQUIRE_KEYS: any = [
    login_id,
    password,
    password_check,
    kor_name,
...
const loginUser = async (req: Request, res: Response) => {
  ...
  const REQUIRED_KEYS: any = { login_id, password };
...
const modifyAccountInfo = async (req: Request, res: Response) => {
  ...
  const essentialKeys: any = { kor_name, eng_name, email, nickname };
  if (!user_id) {
    const error = new Error('NO USER_ID IN REQ');
    error.status = 404;
    throw error;
